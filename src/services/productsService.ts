...
export async function createProduct(product: IProduct) {
	...
	const result = await db.run(
		`INSERT INTO products (product_name, product_brand, product_price, product_availabe, product_discontinued) VALUES(?, ?, ?, ?, ?)`,
		[
			product.name,
			product.brand,
			product.category,
...
export async function updateProduct(productId: number, product: IProduct) {
  ...
  const result = await db.run(
    `UPDATE products 
    SET product_name = ?, product_brand = ?, product_category = ?, product_price = ?, product_availabe = ?, product_discontinued = ?
    WHERE product_id = ?`,
    [
      product.name,
      product.brand,
