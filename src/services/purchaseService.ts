  ...
  try {
    ...
    const result = await db.run(
      `INSERT INTO Purchase (customer_id, purchase_amount, purchase_paid, purchase_paid_date)
       VALUES (?, ?, ?, ?)`,
      [purchase.customerId, purchase.totalAmount, purchase.paid, purchase.paidDate || null]
    );
...
export async function getPurchaseById(purchaseId: number) {
  const db = await connectDB();
  const purchase = await db.get(`SELECT * FROM Purchase WHERE purchase_id = ?`, [purchaseId]);
  const details = await db.all(`SELECT * FROM PurchaseDetails WHERE purchase_id = ?`, [purchaseId]);
...
export async function updatePurchase(purchaseId: number, purchase: IPurchase) {
  ...
  const result = await db.run(
    `UPDATE Purchase
     ...
     WHERE purchase_id = ?`,
    [purchase.customerId, purchase.totalAmount, purchase.paid, purchase.paidDate || null, purchaseId]
  );
